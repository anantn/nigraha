  ...
  <div id="divPageContent">
    ...
    <table border="0" cellpadding="5" cellspacing="0">
      <tr>
        ...
        <td width="619" valign="top" class="rightColumn">
          <?php
							$squery = mysql_query("SELECT * FROM profile_student WHERE accesslvl LIKE 'dpost'");
	                        $fquery = mysql_query("SELECT * FROM profile_faculty WHERE accesslvl LIKE 'dpost'");
		  ?>
          <p class="subHeader"> Existing Posting Users <br />
            <font size="1">Legend: dpost - Department Posting.</font><br />
          </p>
          <p>
            <?php
							echo "<ul>";
							...
							{   echo "<li><a href=\"dusermod.php?t=stud&id=".mysql_result($squery,$i,'userid')."\" onclick=\"return openWin(this);\">".mysql_result($squery, $i, 'fullname').", ".mysql_result($squery, $i, 'accesslvl').", Student(".mysql_result($squery,$i,'userid').")</a></li>";
							}
							echo"</ul><ul>";
							$i = 0;
							$fmax = mysql_num_rows($fquery);
							for($i=0;$i<$fmax;$i=$i+1)
							{	echo "<li><a href=\"dusermod.php?t=fac&id=".mysql_result($squery,$i,'userid')."\" onclick=\"return openWin(this);\">".mysql_result($squery, $i, 'fullname').", ".mysql_result($squery, $i, 'accesslvl').", Faculty(".mysql_result($squery,$i,'userid').")</a></li>";
							}
							echo"</ul>";
			?>
          </p>
          <p class="subHeader"> Manage Ordinary Users </p>
          <p> <a href="dsearch.php" onclick="return openWin(this)">Click here to search for users</a> </p></td>
        <td></td>      </td>
      </tr>
    </table>
  </div>
