...
<?php
	...
	{	if($_POST['type']=="student")
		{	$squery = mysql_query("SELECT * FROM profile_student WHERE userid LIKE CONVERT( _utf8 '%".$_POST['userid']."%' USING latin1 ) COLLATE latin1_general_ci AND fullname LIKE CONVERT( _utf8 '%".$_POST['nam']."%' USING latin1 ) COLLATE latin1_general_ci");
			...
			$i = 0;
			for($i=0;$i<$max;$i++)
			{	echo "<li><a href=\"ausermod.php?t=stud&id=".mysql_result($squery,$i,'userid')."\">".mysql_result($squery, $i, 'fullname').", ".mysql_result($squery, $i, 'accesslvl').", Student(".mysql_result($squery,$i,'userid').")</a></li>";
			}
			echo"</span></ul>";
		}			
		else
		{	$squery = mysql_query("SELECT * FROM profile_faculty WHERE userid LIKE CONVERT( _utf8 '%".$_POST['userid']."%' USING latin1 ) COLLATE latin1_general_ci AND fullname LIKE CONVERT( _utf8 '%".$_POST['nam']."%' USING latin1 ) COLLATE latin1_general_ci");
			...
			$i = 0;
			for($i=0;$i<$max;$i++)
			{	echo "<li><a href=\"ausermod.php?t=fac&id=".mysql_result($squery,$i,'userid')."\">".mysql_result($squery, $i, 'fullname').", ".mysql_result($squery, $i, 'accesslvl').", Faculty(".mysql_result($squery,$i,'userid').")</a></li>";
			}
			echo"</span></ul>";
		}
		echo "</p><span class=\"style1\">If the page is blank, no results were found! <a href=\"search.php\">Click here</a> to search again. You can click on a user's entry to edit his/her access levels.</span>";
	}
	else
	{
?>
<form action="search.php" method="post" name="form1" class="style1">
