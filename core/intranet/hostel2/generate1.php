...
<form method="POST" action="">
  <div align="center">
    <center>
    <table border="1" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="354" height="159">
      <tr>
        <td width="354" height="61" colspan="2"><b>Generate Hostel List</b></td>
      </tr>
      <tr>
        <td width="157" height="35">
        <p align="right">Hostel Name:</td>
        <td width="191" height="35">
      ...
      <tr>
        ...
        <td width="191" height="36"><select size="1" name="Type">
        <option value="1">Single</option>
        <option value="2">Doule</option>
        <option value="3">Triple</option>
        </select></td>
      </tr>
      <tr>
        <td width="354" height="21" colspan="2">
  ...
  {
    $hostelid = $_POST['HostelName'];
    $conn = mysql_connect( $hostname, $username, $password);
    $ret = mysql_select_db($dbname);
    $query = "SELECT * From tblroomkey Where HostelID =".$hostelid;
    if (isset($_POST['Type'])){
      $type = " AND type =".$_POST['Type'];
      $query = $query . $type;
    }
    $ret = mysql_query($query)or die($query."   ".mysql_error ());
    ...
    if ($debug==1){
      echo "$query<br>";
        ...
        {
            $query1 = "Select * From studentroom Where RoomKeyID =".$row["RoomKeyID"];
            if ($debug==1){
                echo "<b>$query1<br></b>";
            }
            $ret1 = mysql_query($query1) or die($query1."   ".mysql_error ());
            while($row1 = mysql_fetch_array($ret1)){
                ////////////////////////////////
                $query2 = "Select * From tblstudents Where ID =".$row1["StudentID"];
                if ($debug==1){
                echo "$query2<br>";
                }
                $ret2 = mysql_query($query2) or die($query2."   ".mysql_error ());
                $row3 = mysql_fetch_array($ret2);
                echo "<b><large>".mysql_affected_rows()."</b></large>";
                echo $row3['StudentName'];
            }
