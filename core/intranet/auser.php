        ...
        <?php
					...
					{	switch($_COOKIE['access'])
						{	case 'sadmin':
							...
							case 'dadmin':
								echo "<li><a href=\"deptcms.php\">Manage Department Content</a></li>";
							case 'post':
								echo "<li><a href=\"mnitpost/addpost.php.php\">Post News/Notices</a></li>";
							case 'user':
								echo "<li><a href=\"logout.php\">Logout</a></li>";
								break;
						}
    ...
    <table border="0" cellpadding="5" cellspacing="0">
      <tr>
        ...
        <td width="619" valign="top" class="rightColumn"><h2>Manage Administrators</h2>
          <?php
							$squery = mysql_query("SELECT * FROM profile_student WHERE accesslvl LIKE '%admin'");
	                        $fquery = mysql_query("SELECT * FROM profile_faculty WHERE accesslvl LIKE '%admin'");
		  ?>
          ...
          <p class="subHeader"> Existing System Administrators <br />
            <font size="1">Legend: sadmin - Super Administrator, admin - Global Administrator, dadmin - Department Administrator.</font><br />
          </p>
          <p>
            <?php
							echo "<ul>";
							...
							{   echo "<li><a href=\"ausermod.php?t=stud&id=".mysql_result($squery,$i,'userid')."\" onclick=\"return openWin(this);\">".mysql_result($squery, $i, 'fullname').", ".mysql_result($squery, $i, 'accesslvl').", Student(".mysql_result($squery,$i,'userid').")</a></li>";
							}
							echo"</ul><ul>";
							$i = 0;
							$fmax = mysql_num_rows($fquery);
							for($i=0;$i<$fmax;$i=$i+1)
							{	echo "<li><a href=\"ausermod.php?t=fac&id=".mysql_result($fquery,$i,'userid')."\" onclick=\"return openWin(this);\">".mysql_result($fquery, $i, 'fullname').", ".mysql_result($fquery, $i, 'accesslvl').", Faculty(".mysql_result($fquery,$i,'userid').")</a></li>";
							}
							echo"</ul>";
			?>
          </p>
          ...
          <p>
            <?php
							...
							$fmax = mysql_num_rows($fquery);
							for($i=0;$i<$fmax;$i=$i+1)
							{	echo "<li><a href=\"ausermod.php?t=fac&id=".mysql_result($fquery,$i,'userid')."\" onclick=\"return openWin(this);\">".mysql_result($fquery, $i, 'fullname').", ".mysql_result($fquery, $i, 'accesslvl').", Faculty(".mysql_result(fsquery,$i,'userid').")</a></li>";
							}
							echo"</ul>";
			?>
          </p>
          <p class="subHeader"> Manage Ordinary Users </p>
          <p> <a href="search.php" onclick="return openWin(this)">Click here to search for users</a> </p></td>
        <td></td>      </td>
      </tr>
    </table>
  </div>
