...
function displayForm($form)
{
	...
	echo $form->input('Student.collegeid', array('label' => 'Student ID'));
	echo $form->input('Student.password', array('label' => 'Service Password'));
	echo $form->end('Submit');
	echo '</fieldset>';
}

echo '<h2>Course Modification for Student</h2>';

/* Decide on which form to show */
if (isset($state)) {
	switch ($state) {
		case 1: displayForm($form); break;
		case 2: echo '<span class="notice">ERROR: The administrative password was invalid. Please try again:</span>';
				displayForm($form); break;
		case 3: echo '<span class="notice">ERROR: The student ID was invalid. Please try again:</span>';
				displayForm($form); break;
		case 4: echo '<span class="notice">Course set successfully updated. You may begin modification for the next student:</span>';
				displayForm($form); break;
	}
} else {
...
echo $form->hidden('Student.modForm', array('value' => 'true'));
echo '</td></tr></table>';
